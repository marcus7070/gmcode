import pytest
from gmcode.geom import Vector, TOLERANCE
import math
...
def test_vector_dot():
    ...
    v0 = Vector(1, 1, 1)
    assert v0.dot(v0) == pytest.approx(3)

    v1 = v0.cross(Vector(0, 0, 1))
    assert v0.dot(v1) == pytest.approx(0)

    v2 = -v0
    assert v2.dot(v0) == pytest.approx(-3)

    assert v2.dot(v2) == pytest.approx(3)


def test_vector_neg():

    v0 = Vector(1, 1, 1)
    assert -v0 == Vector(-1, -1, -1)


def test_vector_tolerance():

    v0 = Vector()
    v1 = Vector(TOLERANCE * 0.9, 0, 0)
    assert v0 == v1

    v2 = Vector(TOLERANCE * 1.1, 0, 0)
    assert v0 != v2

    v3 = Vector(1000, -1000, 1000)
    assert v3 == v3 + v1
    assert v3 != v3 + v2
